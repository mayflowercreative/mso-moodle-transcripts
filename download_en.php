...
$PAGE->set_url('/local/ibq/download_en.php');
...
$PAGE->set_heading($header);
$PAGE->set_url('/local/ibq/download_en.php', array("id"=>$userid));
...
if(isset($userdetail["Date_Admission"])){
	$admission = date(" F Y", $userdetail["Date_Admission"]);
...
if(isset($userdetail["Base_admission"])){
	if (strlen($userdetail["Base_admission"])>0) {
		...
		$label_baseadmission = "Base admission : ";
	} else {
		$base_admission = " ";
		$label_baseadmission = " ";
	}
...
if(isset($userdetail["Graduation"])){
	if ($userdetail["Graduation"] > 0) {
		$graduation = date(" F Y", $userdetail["Graduation"]);
		$graduation2 = date("Y", $userdetail["Graduation"]);
		$label_graduation = "Remise de diplôme : ";	
	} else {
		$graduation = " ";
		$label_graduation = " ";
	}
...
<tr>
	...
	<td align="center">
		...
		<table>
			<tr>
				<td align="right">'. $label_ibq .'</td>
				<td align="left">'. $ibq_no .'</td>
			</tr>
			<tr>
				<td></td>
				<td align="left">'. $idul .'</td>
			</tr>
		</table>
	</td>
	<td align="right"><strong>Transcript</strong><br>
		'. $print_date .'<br>
		<table>
			<tr>
				<td align="right" width="60%">'. $label_admission .'</td>
				<td width="40%">'. $admission .'</td>
			</tr>
			<tr>
...
<tr>
	<td colspan="3"><strong>'. $label_program . $program .'</strong></td>
</tr>
</table>
<div class="emptyspace1"></div>';

if ($graduation2<2015) {
			$header .= "<div class='col-12 t-program-name'><strong>WARNING</STRONG>, this grade book is not official. Only the courses on Moolde are printed</div>";
		}
...
foreach($roles as $key => $role){
	...
	if(count($role)>0){
		...
		$body .= '
		<tr class="t-row-head">
			<td colspan="3" width="70%">'. $rolename .'</td>
			<td colspan="3" width="30%">Number of courses : '. count($role) .'</td>
		</tr>
		';
		...
		foreach($role as $course){
			...
			$body .= '
			<tr>
				<td colspan="3" width="70%">'. $course->fullname .'</td>
...
$avg_comment = $ibq->get_comment_by_letter_en($avg_letter);

$footer = '<div class="emptyspace50"></div>
	...
	<table>
		<tr>
			<td width="30%">
				...
				<tbody>
				<tr class="t-row-head">
					<td align="right" colspan="2">Number of Courses</td>
				</tr>
				<tr>
					<td>Total</td>
			...
			<td width="25%">
				...
				<tbody>
				<tr class="t-row-head">
					<td align="center">Total Credit Attempted</td>
				</tr>
				<tr>
				...
				<tr class="t-row-head">
					<td align="center">Total Credit Earned</td>
				</tr>
				<tr>
		...
		<tr>
			<td colspan="2">
				<div class="t-footer-note">
					<small><strong>Note:</strong> This statement shows the student\'s progress for information purposes. It can be used as an official statement when accompanied by the seal of the Institute and the signature of the Registrar. </small>
				</div>
			</td>
		</tr>
	</table>';
...
//start pdf building
$pdf_name = "Transcript_en_ID$user->id.pdf";
