...
$PAGE->set_url('/local/ibq/fte_bog_fr.php');
...
$PAGE->set_heading($header);
$PAGE->set_url('/local/ibq/fte_bog_fr.php', array("annee"=>$annee,"session" => $session));
...
for ($j = 0; $j <= 20; $j++) 
{
	if ($l_role_name[$j]!="*") {
		...
		$body .= '<th align=center width=15%>'. $l_role_name[$j] .'</th>';
	}
}
$body .= '<th align=center>Total</th>';
...
{
	if ($l_pg2[$i]!="Z") {	
		$ligne = '<tr><td width=60%>'. $l_pg2[$i] .'</td>';
		$i2=-1;
		for ($i3 = 0; $i3 <= 30; $i3++) {
		...
		{
			if ($l_role_name[$j]!="*") {
				$ligne .= '<td align=center>'. $l_tableau [$i2][$j] .'</td>';
				$total+=$l_tableau [$i2][$j];
				$g_total[$j]+=$l_tableau [$i2][$j];	
			}	
		}
		$ligne .= '<td align=center>'. $total .'</td>';
		$ligne .= '</tr>';
		if ($total>0) {
			$body .= $ligne;
		}
...
$body .= '<tr><td><strong>Total</strong></td>';
		...
		{
			if ($l_role_name[$j]!="*") {
				$body .= '<td align=center><strong>'. $g_total[$j] .'<strong></td>';
				$total+=$g_total[$j];
			}
		}
		$body .= '<td align=center><strong>'. $total .'</strong></td>';
		$body .= '</tr>';
...
foreach($sqlcountstudents as $sqlcountstudent)
	{
		if ($sqlcountstudent->nb>0) {
			$body .= $sqlcountstudent->nb ." étudiantes (F) distinctes<BR>";
		}
	}
...
foreach($sqlcountstudents as $sqlcountstudent)
	{
		if ($sqlcountstudent->nb>0) {
			$body .= $sqlcountstudent->nb ." étudiants (M) distincts<BR>";
		}
	}
...
foreach($sqlcountstudents as $sqlcountstudent)
	{
		if ($sqlcountstudent->nb>0) {
			$body .= "<strong>".$sqlcountstudent->nb."</strong> étudiants distincts total<BR><BR>";
		}
	}
...
foreach($sqlstudents as $sqlstudent)
	...
	if ($nombre_de_nouveaux>1){
		$body .= $nombre_de_nouveaux . " nouveaux étudiants<BR><BR>";
	} elseif ($nombre_de_nouveaux==1) {
		$body .= "un nouvel étudiant<BR><BR>";
	}
	...
	for ($j = 0; $j <= 10; $j++) {
		if ($nombre_par_nb_de_cours[$j]>0) {
			if ($nombre_par_nb_de_cours[$j]>1){
				$body .= $nombre_par_nb_de_cours[$j] . " étudiants ont suivi ". $j. " cours<BR>";
			} else {
				$body .= $nombre_par_nb_de_cours[$j] . " étudiant(e) a suivi ". $j. " cours<BR>";
			}
		}
	}
	$body .= "<BR><BR> ";
	...
	{
		if ($l_total_type[$i]>0) {
			$body .=  $l_total_type[$i] . " ". $types_cours[$i]. "<BR>";
			$total += $l_total_type[$i];
		}
	}
	$body .= "<STRONG> ". $total . " cours</STRONG><BR>";
	$body .= "<BR>";
...
$download = new moodle_url($CFG->wwwroot.'/local/ibq/download_bog.php', array("annee" => $annee,"session" => $session));

$html .= '<div class="row">
	<div class="col-12">
		<a class="btn btn-primary" href="'. $back .'">'. get_string("btnhome","local_ibq") .'</a>
		<a class="btn btn-primary" style="margin-left:10px" href="'. $download .'" target="_blank">'. get_string("btndownload_bog","local_ibq") .'</a>
