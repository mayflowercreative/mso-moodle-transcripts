...
$PAGE->set_url('/local/ibq/download_fr.php');
...
$PAGE->set_heading($header);
$PAGE->set_url('/local/ibq/download_fr.php', array("id"=>$userid));
...
if(isset($userdetail["Date_Admission"])){
	...
	$admission = strftime("%B %Y", $userdetail["Date_Admission"]);
...
if(isset($userdetail["Base_admission"])){
	if (strlen($userdetail["Base_admission"])>0) {
		...
		$label_baseadmission = "Base d'admission : ";
	} else {
		$base_admission = " ";
		$label_baseadmission = " ";
	}
...
<tr>
	...
	<td align="right"><strong>Relevé de notes</strong><br>
		'. $print_date .'<br>
		<table>
			<tr>
...
if ($graduation2<2015) {
			$header .= "<div class='col-12 t-program-name'><strong>ATTENTION</STRONG>, ce relevé n'est pas officiel, il ne contient que les cours sous Moodle</div>";
		}
...
foreach($roles as $key => $role){
	...
	if(count($role)>0){
		...
		<tr class="t-row-head">
			...
			<td colspan="3" width="30%">Nombre de cours : '. count($role) .'</td>
		</tr>
		';
...
<h3 class="cr-text-center">Sommaire</h3>
	<table>
		<tr>
			<td width="30%">
				<table class="table" cellpadding="5">
				...
				<tbody>
				<tr class="t-row-head">
					<td align="right" colspan="2">Nombre de cours</td>
				</tr>
				<tr>
					<td>Total</td>
			...
			<td width="25%">
				...
				<tbody>
				<tr class="t-row-head">
					<td align="center">Nombre de crédits inscrits</td>
				</tr>
				<tr>
				...
				<tr class="t-row-head">
					<td align="center">Nombre de crédits achevés</td>
				</tr>
				<tr>
			...
			<td width="25%">
				...
				<tr class="t-row-head">
					<td align="center" colspan="2">Moyenne</td>
				</tr>
				<tr>
		...
		<tr>
			<td colspan="2">
				<div class="t-footer-note">
					<small><strong>Note:</strong> Ce relevé représente le cheminement de l’étudiant à titre d’information. Il peut servir de relevé officiel lorsqu’il est accompagné du sceau de l’Institut et de la signature du registraire. </small>
				</div>
			</td>
		</tr>
	</table>';
...
//start pdf building
$pdf_name = "Transcript_fr_ID$user->id.pdf";
