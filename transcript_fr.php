...
$PAGE->set_url('/local/ibq/transcript_fr.php');
...
$PAGE->set_heading($header);
$PAGE->set_url('/local/ibq/transcript_fr.php', array("id"=>$userid));
...
foreach($roles as $key => $role){
	...
	if(count($role)>0){
		...
		<tr class="t-row-head">
			...
			<td colspan="4">Nombre de cours : '. count($role) .'</td>
		</tr>
		';
...
$footer = '
	<div class="row">
		...
		<div class="col-4">
			...
			<tr class="t-row-head">
				<td></td>
				<td align="center">Nombre de cours</td>
			</tr>
			<tr>
				<td>Total</td>
...
$download = new moodle_url($CFG->wwwroot.'/local/ibq/download_fr.php', array("id" => $userid));
$edituser = new moodle_url($CFG->wwwroot.'/user/editadvanced.php', array("id" => $userid, "course"=> 1));
$viewtranscript_en = new moodle_url($CFG->wwwroot.'/local/ibq/transcript_en.php', array("id" => $userid));
...
$html .= '<div class="row">
	<div class="col-12">
		...
		<a class="btn btn-primary" style="margin-left:10px" href="'. $viewtranscript_en .'" >'. get_string("btntranscript_en","local_ibq") .'</a>
	</div>
</div>
<hr>
<div class="container">
